...
export default function AdventureGame({kid,progress,settings,onObjective,onExit,onTreasure,onBoss}:Props){
  ...
  useEffect(()=>{
    ...
    const width=Math.min(1180,Math.max(760,window.innerWidth-28))
    const height=Math.min(720,Math.max(540,window.innerHeight-168))
  ...
  if(fallback)return <main className="page-shell adventure-fallback"><header className="page-header"><button className="secondary" onClick={onExit}>← Adventure Hub</button><div><h1>{zone.icon} {zone.name}</h1><p>Accessible adventure fallback</p></div></header><section className="fallback-map">{zone.checkpoints.map((c,i)=><button key={c} onClick={()=>onObjective(`cp-${zone.id}-${i}`,`Explored ${c}`)}>✦ {c}</button>)}{zone.npcs.map(n=><button key={n.id} onClick={()=>onObjective(`npc-${zone.id}-${n.id}`,`Met ${n.name}`)}>{n.icon} {n.name}</button>)}{zone.shipwrecks.flatMap(s=>s.rooms.map((r,i)=><button key={`${s.id}-${r}`} onClick={()=>onObjective(`room-${zone.id}-${s.id}-${i}`,`Searched ${r}`)}>◇ {s.name}: {r}</button>))}<button onClick={onTreasure}>🧰 Search for Treasure</button>{bossReady&&<button onClick={onBoss}>🏆 Begin Mastery Trial</button>}</section></main>

  return <main className="game-page game-page-v10">
    <section className="adventure-topbar">
      <button className="adventure-back" onClick={onExit}>← Hub</button>
      <div className="explorer-identity"><span className="explorer-avatar">{progress.avatar||'🧭'}</span><div><strong>{kid}</strong><small>Level {progress.adventure.level} Explorer</small><i><b style={{width:pct(progress.adventure.xp%180,180)}}/></i></div></div>
      <div className="adventure-resource"><span>⚡</span><div><small>Adventure Energy</small><strong>{progress.adventure.energy} / 12</strong></div></div>
      <div className="adventure-resource"><span>🦪</span><div><small>Pearls</small><strong>{progress.adventure.pearls.toLocaleString()}</strong></div></div>
      <div className="adventure-resource"><span>🐚</span><div><small>Shells</small><strong>{progress.adventure.shells.toLocaleString()}</strong></div></div>
    </section>

    <section className="adventure-stage-shell">
      <aside className="adventure-sidepanel mission-panel">
        <span className="panel-kicker">CURRENT MISSION</span>
        <h2>🧭 {activeMission?.title||'Ocean Expedition'}</h2>
        <p>{activeMission?.description||zone.story}</p>
        {activeMission&&<><div className="hud-progress"><i style={{width:pct(activeMission.progress,activeMission.target)}}/></div><small>{Math.min(activeMission.progress,activeMission.target)} / {activeMission.target}</small></>}
        <hr/>
        <span className="panel-kicker">REGION</span><h3>{zone.icon} {zone.name}</h3><p>{zone.story}</p>
        <div className="hud-progress green"><i style={{width:`${exploration}%`}}/></div><small>{exploration}% explored · {objectiveCount}/{totalObjectives} discoveries</small>
      </aside>

      <div className="adventure-canvas-wrap">
        <div ref={host} className="phaser-host phaser-host-v10"/>
        <div className="adventure-caption">{selected}</div>
      </div>

      <aside className="adventure-sidepanel discovery-panel">
        <span className="panel-kicker">EXPLORE & DISCOVER</span>
        <h2>{zone.shipwrecks[0]?.name||zone.name}</h2>
        <div className="discovery-stat"><span>✦</span><div><small>Secrets Found</small><strong>{objectiveCount} / {totalObjectives}</strong></div></div>
        <div className="discovery-icons">{Array.from({length:Math.min(6,totalObjectives)},(_,i)=><span key={i} className={i<objectiveCount?'found':''}>{i<objectiveCount?'✓':'?'}</span>)}</div>
        <hr/>
        <div className="discovery-stat"><span>🧰</span><div><small>Treasure Caches</small><strong>{progress.adventure.treasureClaimed.filter(id=>id.includes(zone.id)).length}</strong></div></div>
        {zone.boss&&<div className={`boss-hud ${bossReady?'ready':''} ${bossDefeated?'complete':''}`}><span>{bossDefeated?'🏆':zone.boss.icon}</span><div><small>{zone.boss.name}</small><strong>{bossDefeated?'Complete':bossReady?'Ready':'Locked'}</strong></div>{bossReady&&<button onClick={onBoss}>Start</button>}</div>}
      </aside>
    </section>

    <section className="adventure-bottom-hud">
      <div><span>🗺️</span><small>World 1</small><strong>{zone.name}</strong></div>
      <div><span>📜</span><small>Story Chapter</small><strong>{progress.adventure.storyChapter} / 8</strong></div>
      <div><span>🧠</span><small>Learning Powers</small><strong>XP + Energy</strong></div>
      <div><span>🏆</span><small>Boss Trials</small><strong>{progress.adventure.bossesDefeated.length} Complete</strong></div>
    </section>
  </main>
}
